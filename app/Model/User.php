...
class User extends AppModel {
    ...
    function isUnqiueUserName() {
        ...
        if (isset($this->data['User']['username']) && !empty($this->data['User']['username'])) {
            $userCount = $this->find('count', array('conditions' => array('User.username' => $this->data['User']['username']), 'contain' => false));
            if ($userCount > 0) {
                $valid = false;
            }
        }
    ...
    function isUnqiueEmail() {
        ...
        if (isset($this->data['User']['email']) && !empty($this->data['User']['email'])) {
            $emailCount = $this->find('count', array('conditions' => array('User.email' => $this->data['User']['email']), 'contain' => false));
            if ($emailCount > 0) {
                $valid = false;
            }
        }
