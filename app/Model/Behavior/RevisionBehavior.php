...
class RevisionBehavior extends ModelBehavior {
	 ...
	 * @var array
	 */
	private $defaults = array('limit' => false, 'auto' => true, 'ignore' => array(), 'useDbConfig' => null, 'model' => null);
	/**
	...
	public function createRevision(&$Model) {
		...
		$data = $Model -> find('first', array('conditions' => array($Model -> alias . '.' . $Model -> primaryKey => $Model -> id), 'contain' => $habtm));
	...
	private function init(&$Model, $page, $limit) {
		...
		$all = $Model -> find('all', array('limit' => $limit, 'page' => $page, 'contain' => $habtm));
	...
	public function revertToDate(&$Model, $datetime, $cascade = false, $force_delete = false) {
		...
		if ($cascade) {
			...
			foreach ($associated as $assoc => $data) {
				...
				$revision_children = $Model -> $assoc -> ShadowModel -> find('all', array('fields' => array('DISTINCT ' . $Model -> primaryKey), 'conditions' => array($data['foreignKey'] => $Model -> id, 'NOT' => array($Model -> primaryKey => $ids)), ));
		...
		$data = $Model -> ShadowModel -> find('first', array('conditions' => array($Model -> primaryKey => $Model -> id, 'version_created <=' => $datetime), 'order' => 'version_created ASC, version_id ASC'));
		...
		$liveData = $Model -> find('first', array('contain' => $habtm, 'conditions' => array($Model -> alias . '.' . $Model -> primaryKey => $Model -> id)));
	...
	public function beforeDelete(&$Model) {
		...
		foreach ($Model->hasAndBelongsToMany as $assocAlias => $a) {
			if (isset($Model -> {$assocAlias} -> ShadowModel -> _schema[$Model -> alias])) {
				$joins = $Model -> {$a['with']} -> find('all', array('recursive' => -1, 'conditions' => array($a['foreignKey'] => $Model -> id)));
