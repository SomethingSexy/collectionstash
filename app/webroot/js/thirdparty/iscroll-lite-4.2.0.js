...
(function() {
	...
	// Events
	RESIZE_EV = 'onorientationchange' in window ? 'orientationchange' : 'resize', START_EV = hasTouch ? 'touchstart' : 'mousedown', MOVE_EV = hasTouch ? 'touchmove' : 'mousemove', END_EV = hasTouch ? 'touchend' : 'mouseup', CANCEL_EV = hasTouch ? 'touchcancel' : 'mouseup',
	...
	iScroll.prototype = {
		...
		_start : function(e) {
			var that = this, point = hasTouch ? e.touches[0] : e, matrix, x, y;
		...
		_move : function(e) {
			var that = this, point = hasTouch ? e.touches[0] : e, deltaX = point.pageX - that.pointX, deltaY = point.pageY - that.pointY, newX = that.x + deltaX, newY = that.y + deltaY, timestamp = e.timeStamp || Date.now();
		...
		_end : function(e) {
			...
			var that = this, point = hasTouch ? e.changedTouches[0] : e, target, ev, momentumX = {
				...
				dist : 0,
				time : 0
			}, duration = (e.timeStamp || Date.now()) - that.startTime, newPosX = that.x, newPosY = that.y, newDuration;
