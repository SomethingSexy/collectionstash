	...
	var LP = {
		...
		_setupHorizontalMenu : function() {
			...
			$('<a href="#" id="lp-active" />').prependTo(left_panel).bind('click', function() {
				...
				return false;
			}).text($(($('.active', left_panel).length == 1) ? '.active' : '.active .active', left_panel).text());
		},
		...
		_calculateDropdownWidth : function(id) {
			var width = 0;
			if (LP._dropdowns[id].menu.hasClass('simple')) {
				var parent = LP._dropdowns[id].menu.parent(), div = $('<div id="calculateDropdownWidth" />').css({
					position : 'absolute',
					width : 3000,
					left : -10000
				}).appendTo('body').append(LP._dropdowns[id].menu);
		...
		_setDropdownTopPosition : function(id) {
			...
			if (d.menu.hasClass('simple')) {
				var parent = d.toggler.parents('li'), parent_top = parent.offset().top, parent_height = parent.height(), wrapper_height = d.wrapper.height(), window_height = $(window).height(), scroll_top = $(document).scrollTop();
