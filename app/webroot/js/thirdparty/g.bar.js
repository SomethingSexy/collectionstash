	...
	function VBarchart(paper, x, y, width, height, values, opts) {
		...
		for (var i = 0; i < len; i++) {
			...
			for (var j = 0; j < (multi || 1); j++) {
				var h = Math.round(( multi ? values[j][i] : values[i]) * Y), top = y + height - barvgutter - h, bar = finger(Math.round(X + barwidth / 2), top + h, barwidth, h, true, type, null, paper).attr({
					stroke : "none",
			...
			if (opts.stacked) {
				for (var i = 0; i < len; i++) {
					...
					for (var j = 0; j < (multi || 1); j++) {
						...
						if (j == multi - 1) {
							...
							L = paper.text(bars[j][i].x, y + height - barvgutter / 2, label).attr(chartinst.txtattr).attr({
				...
				for (var i = 0; i < len; i++) {
					for (var j = 0; j < (multi || 1); j++) {
						...
						L = paper.text(bars[j][i].x - barwidth / 2, isBottom ? y + height - barvgutter / 2 : bars[j][i].y - 10, label).attr(chartinst.txtattr).attr({
							fill : opts.legendcolor || "#000",
							"text-anchor" : "start"
						}).insertBefore(covers[i * (multi || 1) + j]);
		...
		chart.label = function(labels, isRight) {
			...
			for (var i = 0; i < len; i++) {
				for (var j = 0; j < multi; j++) {
					var label = chartinst.labelise( multi ? labels[j] && labels[j][i] : labels[i], multi ? values[j][i] : values[i], total), X = isRight ? bars[j][i].x - barheight / 2 + 3 : x + 5, A = isRight ? "end" : "start", L;

					this.labels.push( L = paper.text(X, bars[j][i].y, label).attr(chartinst.txtattr).attr({
						fill : opts.legendcolor || "#000",
						"text-anchor" : "start"
