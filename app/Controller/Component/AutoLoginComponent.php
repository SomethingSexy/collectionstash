...
class AutoLoginComponent extends Component {
	...
	public function startup(Controller $controller) {
		...
		if (!$this -> active || !empty($user) || !$controller -> request -> is('get')) {
			...
			CakeLog::write('error', 'cookieFail' . date("Y-m-d H:i:s", time()));
			...
			CakeLog::write('error', 'hashFail' . date("Y-m-d H:i:s", time()));
			$this -> delete();
			return;
		}
		...
		if ($this -> Auth -> login()) {
			...
			CakeLog::write('error', 'loginFail' . date("Y-m-d H:i:s", time()));
	...
	public function debug($key, $cookie = array(), $user = array()) {
		$scopes = array('login' => 'Login Successful', 'loginFail' => 'Login Failure', 'loginCallback' => 'Login Callback', 'logout' => 'Logout', 'logoutCallback' => 'Logout Callback', 'cookieSet' => 'Cookie Set', 'cookieFail' => 'Cookie Mismatch', 'hashFail' => 'Hash Mismatch', 'custom' => 'Custom Callback');
